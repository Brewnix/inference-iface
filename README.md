...
- [Hypermesh preempt v0](docs/hypermesh-preempt-v0.md) — locked Host preempt (`hypermesh.lease_stop` / `hypermesh.sell_pause`); Brewnix proposes, Host executes; **H3 drain executor** sequences two CP jobs (not a Host mega-job); **H4** offline owner unix socket on Host **main**; tools **never** implied by `rails_profile`; H1+H2+H4 on Host **main**; Panopticon **#40** site jobs + **#41** site `sell_state` read on **main**; no `schemas/` change

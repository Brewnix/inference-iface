...
- [Hypermesh preempt v0](docs/hypermesh-preempt-v0.md) — locked Host preempt (`hypermesh.lease_stop` / `hypermesh.sell_pause`); Brewnix proposes, Host executes; **H3 drain executor** sequences two CP jobs (not a Host mega-job); tools **never** implied by `rails_profile`; H1+H2 on Host **main**; no `schemas/` change

...
**Companion:** [notify.operator ↔ shared auditor door](notify-operator-audit-door.md) (site half) · [fyber.privilege_grant v0](privilege-grant-v0.md) (incident elevation; **not** this ticket) · [fyber.incident binding v0](incident-binding-v0.md) (overlay case; tickets stay historical via side index) · [Zero-LLM OPNsense detect → block → receipt loop](zero-llm-opnsense-loop.md) · [SociACL IR binding v0](sociacl-ir-binding-v0.md) (Check + `delegate` on `:ir`; hop/URL never a grant; site token out of SociACL v0)

Goal: a **shared prompt-router / auditor API** on the Hypermesh / Panopticon plane. Sites already write a local `fyber.receipt/v0` and POST a typed `notify.operator`. This doc is the plane door those POSTs hit. The plane is a human inbox + resolution **intent**. Actuation truth stays on the site receipt. A ticket may **approve a grant**; a grant is **not** a one-shot held-tool approve — that stays on this door (`asks: []` is a ticket, not `fyber.privilege_grant/v0`). Empty-ask one-shot approve does **not** require an [`incident_id`](incident-binding-v0.md). Tickets that sit on a case stay historical via the side index (or a later additive field); do not add `incident_id` to this ticket contract in v0.
